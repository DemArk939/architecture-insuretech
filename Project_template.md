...
### Решения:
1. Требуется ли своё хранилище данных? - Да, БД будем использовать для сохранения предложений для клиента.
Информацию будем использовать для передачи в core-app при оформлении страховки, а также для передачи клиенту
в случае сбоя и повторного запроса от него. Также нужно обеспечить отчистку данных и определиться с временем хранения.
2. Какой API сервис предоставляет core-app? - Сервис будет предоставлять функционал для непосредственного оформления страховки.
Для этого нужно передать на вход данные по клиенту и выбранное клиентом предложение от страховой.
3. Определите средство интеграции между сервисами core-app и osago-aggregator - core-app будет получать данные по REST.
4. Подумайте над API для веб-приложения в osago-aggregator - Если клиент зарегистрирован, и в системе имеются все необходимые данные,
то для передачи на сервер будет достаточно ИД пользователя и информации об автомобили. Если же клиент новый, то потребуется
передача на сервер всей необходимой информации для расчета предложений.
5. Определите средство интеграции между веб-приложением и osago-aggregator. - SSE технологии должно быть достаточно для решения задачи.
В случае наличия ограничений, можно рассмотреть WebSockets как альтернативу.
Требуется ли где-то применение паттернов отказоустойчивости:
- Rate Limiting - необходимо использовать для контроля количества запросов от клиентов, чтобы избежать сбоя системы
- Circuit Breaker - подключить на уровне istio, для избегания каскадных сбоев и проблем с внешними сервисами или компонентами
- Retry - обрабатывает временные сбои, подключить на уровне istio внутри неймспейса между сервисами, 
также можно использовать и для внешних запросов, но с осторожностью
